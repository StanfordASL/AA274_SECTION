...
Since we downloaded a new catkin package, we need to rebuild the workspace by running the following from the \texttt{~/catkin\_ws} directory (or if you're using Docker, then \texttt{cd} to the \texttt{aa274-docker} directory and prepend the following command with \texttt{./run.sh}, as usual).
