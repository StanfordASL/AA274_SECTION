...
Our goals for this section: \begin{enumerate}
    ...
    \item Introduce yourselves to dynamic reconfigure.
\end{enumerate}
...
{\bf Already have the package:} Great! You can move on to the next section.
...
Once the package has finished cloning, we need to rebuild our catkin workspace. Recall, we can do this as follows
\begin{lstlisting}
...
{\bf NOTE:} If you are using Docker, then you will need to use the \texttt{display} flag here, since \texttt{rviz} uses a graphical user interface (GUI). Specifically, from the \texttt{aa274-docker} directory, you'd run 
\begin{lstlisting}
./run.sh --display 1
rostb3 # Remember to change the turtlebot host in this file!!
rviz
\end{lstlisting}
...
\subsubsection{Raspberry Pi Camera's Image}
